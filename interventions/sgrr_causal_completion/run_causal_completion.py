...
MANIFEST_PATH = Path(
    os.environ.get("THESIS_SGRR_MANIFEST", str(SOURCE_ROOT / "SOURCE_MANIFEST.json"))
).resolve()
CONFIG = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
CONTRACT = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
LEGACY_ROOT = Path(CONFIG["legacy_mechanism_root"])
    ...
    for record in manifest.get("files", []):
        ...
        if record.get("verify_nested_files"):
            ...
            for nested_record in nested.values():
                nested_path = Path(nested_record["path"])
...
class CompletionRuntime(legacy.SeedRuntime):
    ...
    def _load_calibration(self) -> dict[str, np.ndarray]:
        ...
        if not path.is_file():
            path = SOURCE_ROOT / "calibration" / f"seed{self.seed}.npz"
        if not path.is_file():
