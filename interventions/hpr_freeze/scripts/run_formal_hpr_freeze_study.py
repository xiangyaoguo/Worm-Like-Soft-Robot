...
DATA_ROOT = Path(
    os.environ.get("THESIS_HPR_OUTPUT", str(PROJECT_ROOT / "outputs" / "hpr_freeze"))
).resolve() / "data"
OFFICIAL_EVALUATIONS_ROOT = (
    PROJECT_ROOT / "provenance" / "formal_parent_exact" / "formal" / "evaluations"
)
MECHANISM_RUNTIME_ROOT = PROJECT_ROOT / "interventions" / "mechanism_runtime"
...
for import_path in (MECHANISM_RUNTIME_ROOT,):
...
EXPECTED_CHECKPOINT_HASHES = {
    ...
    9203: "0f09b4bcbc6df1ad981f0782984e1530790e860b2d7857d28876100d2332e6a9",
...
def compare_baseline_to_official(training_seed: int, episodes: list[dict[str, Any]]) -> dict[str, Any]:
    official_path = OFFICIAL_EVALUATIONS_ROOT / f"formal__seed{training_seed}__R0__eval_attempt1.json"
    ...
    expected_success_count = sum(
        bool(kinematic_success(row)) for row in official_episodes
    )
    actual_success_count = sum(
        bool(row["success_kinematic"]) for row in episodes
    )
    if actual_success_count != expected_success_count:
        raise RuntimeError(
            f"Formal HPR baseline success-count mismatch for seed {training_seed}: "
            f"{actual_success_count} != archived {expected_success_count}"
        )
    return {
        "official_evaluation": str(official_path),
        "episode_count": EPISODES,
        "official_common_criterion_success_count": expected_success_count,
        "actual_common_criterion_success_count": actual_success_count,
        "all_common_criterion_success": actual_success_count == EPISODES,
...
def main() -> None:
    ...
    if any(seed not in EXPECTED_CHECKPOINT_HASHES for seed in seeds):
        raise ValueError(f"Only formal HPR seeds 9201, 9203, and 9205 are permitted: {seeds}")
    started = time.perf_counter()
