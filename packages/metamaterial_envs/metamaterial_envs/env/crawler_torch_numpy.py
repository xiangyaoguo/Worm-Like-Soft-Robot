...
observation_funcs = {}
def observation_func(dim=1, mag=torch.pi):
    def decorator(func):
        name = func.__name__
...
class env(EnvBase):
    ...
    def _reset(self, tensordict):
        ...
        self.thdot = torch.zeros((self.num_envs, self.num_particles,), device=self.device, dtype=torch.float32)
        self.camera_pos = 0j
        self.camera_vel = 0.
    ...
    def _step(self, tensordict):
        ...
        self.pos, self.vel, self.thdot = torch.tensor(np_pos, device=self.device), torch.tensor(np_vel, device=self.device), torch.tensor(np_thdot, device=self.device, dtype=torch.float32)
...
def get_obs_dth_tot(self):
    ...
    dth_tot = dthP-dthM
    return dth_tot[:,1:-1].unsqueeze(-1)
...
def get_obs_dth_neighbours(self):
    ...
    dthP=torch.roll(dth,(0, -1), (0, 1))
    dthM=torch.roll(dth,(0, 1), (0, 1))
    return torch.stack((dthP[:,1:-1], dthM[:,1:-1]), dim=2)

@observation_func(dim=4, mag=100)
def get_obs_dth_and_thdot_neighbours(self):
    dth = internode_angle(self.pos) - self.angle_eq
    dth[:,0] = 0.
    dth[:,-1] = 0.
    dthP=torch.roll(dth,(0, -1), (0, 1))
    ...
    obs = torch.stack((dthP[:,1:-1], dthM[:,1:-1], thdotP[:,1:-1], thdotM[:,1:-1]), dim=2)
    return obs
...
def get_obs_dth_neighbours_plus_own(self):
    ...
    dthP=torch.roll(dth,(0, -1), (0, 1))
    dthM=torch.roll(dth,(0, 1), (0, 1))
    return torch.stack((dthP[:,1:-1], dth, dthM[:,1:-1]), dim=2)
...
def wall_force(pos: np.ndarray, vel: np.ndarray, ks: float, gs: float, rad: float):
    ...
    for i in range(num_envs): # numba doesn't support multi-dim indexing mask :(
        msk = np.imag(pos[i]) < rad
        fw[i][msk] = -gs * vel[i][msk] - 1j * ks * (np.imag(pos[i][msk]) - rad)
