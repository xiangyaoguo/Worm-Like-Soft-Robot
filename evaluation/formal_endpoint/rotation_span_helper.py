...
from r0_v21core_contract import (
    REWARD_CONTRACT as R0_V21CORE_REWARD_CONTRACT,
    REWARD_CONTRACT_SHA256 as R0_V21CORE_REWARD_CONTRACT_SHA256,
    REWARD_ID as R0_V21CORE_REWARD_ID,
    contract_sha256,
)
...
DEFAULT_REPO = Path(
    r"C:\Users\PUBLIC_USER\CloudStorage\Desktop\finalproject\job"
    r"\RLMetamaterialLocomotion-main\RLMetamaterialLocomotion-main"
)
...
def _support_diagnostics(env: Any, td: Any) -> tuple[float | None, float | None]:
    """Read support diagnostics, computing them directly for simple rewards.

    Reward-only baselines do not export the fast-forward log fields.  Calling
    the environment's metric routine here is evaluation-only: it does not
    alter the policy input or the trained checkpoint, and gives every reward
    arm the same contact-aware pulse detector.
    """

    support = _log_info_scalar(td, "fast_forward_support_index")
    strength = _log_info_scalar(td, "fast_forward_ground_contact_strength")
    if support is not None and strength is not None:
        return support, strength

    compute = getattr(env, "_compute_fast_forward_roll_v2_metrics", None)
    if not callable(compute):
        return support, strength
    metrics = compute()
    if support is None:
        support = _finite_scalar(metrics.get("fast_forward_support_index"))
    if strength is None:
        strength = _finite_scalar(metrics.get("fast_forward_ground_contact_strength"))
    return support, strength
...
def _rotation_excursion_metrics(
    rotation_increments: np.ndarray,
    threshold_radians: float = 2.0 * math.pi,
) -> dict[str, Any]:
    """Measure the largest ordered best-fit rotation excursion in either direction."""
    increments = np.asarray(rotation_increments, dtype=np.float64).reshape(-1)
    if not np.all(np.isfinite(increments)):
        raise ValueError("rotation increments must be finite")
    cumulative = np.r_[0.0, np.cumsum(increments)]

    running_min = float(cumulative[0])
    running_min_step = 0
    running_max = float(cumulative[0])
    running_max_step = 0
    best = 0.0
    best_start = 0
    best_end = 0
    ...
    tolerance = 1e-10
    for end_step in range(1, len(cumulative)):
        value = float(cumulative[end_step])
        positive = value - running_min
        negative = running_max - value
        if positive > best:
            best = positive
            best_start = running_min_step
            best_end = end_step
            best_direction = "ccw"
        if negative > best:
            best = negative
            best_start = running_max_step
            best_end = end_step
            best_direction = "cw"
        if first_end is None and max(positive, negative) >= threshold_radians - tolerance:
            first_end = end_step
            if positive >= negative:
                first_start = running_min_step
                first_direction = "ccw"
            else:
                first_start = running_max_step
                first_direction = "cw"
        if value < running_min:
            running_min = value
            running_min_step = end_step
        if value > running_max:
            running_max = value
            running_max_step = end_step

    range_radians = float(np.max(cumulative) - np.min(cumulative))
    max_abs_from_start = float(np.max(np.abs(cumulative)))
    return {
        "loose_360_detected": bool(range_radians >= threshold_radians - tolerance),
        "best_fit_rotation_range_degrees": float(np.degrees(range_radians)),
        "max_abs_rotation_from_episode_start_degrees": float(
            np.degrees(max_abs_from_start)
        ),
        "max_rotation_excursion_degrees": float(np.degrees(best)),
        "max_rotation_start_step": int(best_start),
        "max_rotation_end_step": int(best_end),
        "max_rotation_direction": best_direction,
        "first_360_start_step": first_start,
        "first_360_completion_step": first_end,
        "first_360_direction": first_direction,
        "max_abs_step_rotation_degrees": float(
            np.degrees(np.max(np.abs(increments))) if len(increments) else 0.0
        ),
    }
...
def _episode_metrics(
    ...
    excursion = _rotation_excursion_metrics(raw_rotation)
    ...
    return {
        ...
        "forward_speed_x100": np.float64(100.0 * displacement / max(steps, 1)),
        **excursion,
...
def evaluate_checkpoint(
    ...
    k1_sign_constraint = str(
        _metadata_value(metadata, "k1_sign_constraint", "none")
    )
    canonical_k1_signs = np.asarray(
        _metadata_value(
            metadata,
            "k1_sign_by_joint",
    ...
    if canonical_k1_signs.shape != (8,):
        raise RuntimeError(
            f"Unexpected K1 sign vector in checkpoint metadata: {canonical_k1_signs}"
        )
    ...
    environment_module = sys.modules.get(type(env).__module__)
    environment_module_path = Path(
        getattr(environment_module, "__file__", "")
    ).resolve()
    expected_environment_module_path = (
        args.repo.resolve()
        / "metamaterial_envs"
        / "metamaterial_envs"
        / "env"
    ...
    if environment_module_path != expected_environment_module_path:
        close = getattr(env, "close", None)
        if callable(close):
            close()
        raise RuntimeError(
            "Evaluator imported the wrong environment module: "
            f"actual={environment_module_path}, expected={expected_environment_module_path}."
        )
    ...
    actual_k1_sign_constraint = str(
        getattr(env, "k1_sign_constraint", "missing")
    )
    k1_sign_environment_validated = (
        actual_k1_sign_constraint == k1_sign_constraint
    )
    if k1_sign_environment_validated and k1_sign_constraint == "canonical_j01_pos_rest_neg_abs":
        actual_k1_signs = np.asarray(
            getattr(env, "k1_sign_by_joint", []), dtype=np.float64
        )
        k1_sign_environment_validated = (
            actual_k1_signs.shape == canonical_k1_signs.shape
            and np.array_equal(actual_k1_signs, canonical_k1_signs)
        )
    if not k1_sign_environment_validated:
        close = getattr(env, "close", None)
        if callable(close):
            close()
        raise RuntimeError(
            "Evaluator environment K1 sign constraint does not match checkpoint metadata: "
            f"environment={actual_k1_sign_constraint!r}, "
            f"checkpoint={k1_sign_constraint!r}."
        )
    checkpoint_reward_func = str(_metadata_value(metadata, "reward_func", "missing"))
    actual_reward_func = str(getattr(env, "reward_func", "missing"))
    if actual_reward_func != checkpoint_reward_func:
        close = getattr(env, "close", None)
        if callable(close):
            close()
        raise RuntimeError(
            "Evaluator environment reward does not match checkpoint metadata: "
            f"environment={actual_reward_func!r}, checkpoint={checkpoint_reward_func!r}."
        )
    checkpoint_reward_contract = _metadata_value(
        metadata, "roll_stage_reward_contract", None
    )
    checkpoint_reward_contract_sha256 = _metadata_value(
        metadata, "roll_stage_reward_contract_sha256", None
    )
    if checkpoint_reward_contract is None:
        reward_contract_validated = checkpoint_reward_contract_sha256 is None
    else:
        reward_contract_validated = (
            contract_sha256(checkpoint_reward_contract)
            == checkpoint_reward_contract_sha256
        )
    if checkpoint_reward_func == R0_V21CORE_REWARD_ID:
        reward_contract_validated = bool(
            isinstance(checkpoint_reward_contract, dict)
            and checkpoint_reward_contract == R0_V21CORE_REWARD_CONTRACT
            and checkpoint_reward_contract_sha256
            == R0_V21CORE_REWARD_CONTRACT_SHA256
            and reward_contract_validated
        )
    if checkpoint_reward_func == R0_V21CORE_REWARD_ID and not reward_contract_validated:
        close = getattr(env, "close", None)
        if callable(close):
            close()
        raise RuntimeError("R0_V21core_abs360_v1 reward contract hash mismatch.")
    checkpoint_k_action_scale = float(
        _metadata_value(metadata, "k_action_scale", 1.0)
    )
    actual_k_action_scale = float(
        getattr(env, "formula_action_scale", getattr(env, "k_action_scale", math.nan))
    )
    if not math.isclose(
        actual_k_action_scale,
        checkpoint_k_action_scale,
        rel_tol=0.0,
        abs_tol=1e-12,
    ):
        close = getattr(env, "close", None)
        if callable(close):
            close()
        raise RuntimeError(
            "Evaluator environment K action scale does not match checkpoint metadata: "
            f"environment={actual_k_action_scale}, checkpoint={checkpoint_k_action_scale}."
        )
    environment_contract = {
        "checkpoint_k1_sign_constraint": k1_sign_constraint,
        "environment_k1_sign_constraint": actual_k1_sign_constraint,
        "checkpoint_k1_sign_template_active": k1_sign_constraint != "none",
        "environment_k1_sign_template_active": actual_k1_sign_constraint != "none",
        "checkpoint_k1_sign_by_joint": canonical_k1_signs.tolist(),
        "environment_k1_sign_by_joint": np.asarray(
            getattr(env, "k1_sign_by_joint", []), dtype=np.float64
        ).tolist(),
        "k1_sign_reconstruction_validated": bool(k1_sign_environment_validated),
        "environment_module_path": str(environment_module_path),
        "expected_environment_module_path": str(expected_environment_module_path),
        "observation_spec_shape": list(
            env.observation_spec["agents", "observation"].shape
        ),
        "action_spec_shape": list(env.action_spec["agents", "action"].shape),
        "checkpoint_k_action_scale": checkpoint_k_action_scale,
        "environment_k_action_scale": actual_k_action_scale,
        "environment_k1_bounds": [float(env.k1_min), float(env.k1_max)],
        "environment_k2_bounds": [float(env.k2_min), float(env.k2_max)],
        "environment_fix_k1": bool(
            getattr(env, "formula_fix_k1", getattr(env, "fix_k1", False))
        ),
        "environment_fix_k2": bool(
            getattr(env, "formula_fix_k2", getattr(env, "fix_k2", False))
        ),
        "checkpoint_reward_func": checkpoint_reward_func,
        "environment_reward_func": actual_reward_func,
        "checkpoint_reward_contract_sha256": checkpoint_reward_contract_sha256,
        "canonical_reward_contract_sha256": (
            R0_V21CORE_REWARD_CONTRACT_SHA256
            if checkpoint_reward_func == R0_V21CORE_REWARD_ID
            else None
        ),
        "reward_contract_validated": bool(reward_contract_validated),
        "rolling_observation_size": int(getattr(env, "rolling_observation_size", -1)),
        "tail_roll_observation_size": int(getattr(env, "tail_roll_observation_size", -1)),
        "fast_forward_observation_size": int(getattr(env, "fast_forward_observation_size", -1)),
        "scratch_wr_v2_observation_size": int(getattr(env, "scratch_wr_v2_observation_size", -1)),
        "environment_formula_action_names": list(
    ...
    k_action_scale = checkpoint_k_action_scale
    ...
    try:
        for episode_index in range(args.episodes):
            ...
            initial_support, initial_strength = _support_diagnostics(env, td)
            exported_support_index = [initial_support]
            exported_contact_strength = [initial_strength]
            exported_mesh_floor_contact = [
                _log_info_scalar(td, "terrain_floor_contact_strength")
            ]
            raw_k1_steps: list[np.ndarray] = []
            applied_k1_steps: list[np.ndarray] = []
            raw_k2_steps: list[np.ndarray] = []
            for _ in range(args.steps):
                action_td = choose_action(policy, td, "deterministic")
                action_array = (
                    action_td["agents", "action"]
                    .detach()
                    .cpu()
                    .numpy()
                )
                raw_k1 = np.asarray(action_array[0, :, 0], dtype=np.float64) * k_action_scale
                raw_k2 = np.asarray(action_array[0, :, 1], dtype=np.float64) * k_action_scale
                if k1_sign_constraint == "canonical_j01_pos_rest_neg_abs":
                    applied_k1 = np.abs(raw_k1) * canonical_k1_signs
                elif k1_sign_constraint == "none":
                    applied_k1 = raw_k1
                else:
                    raise RuntimeError(
                        f"Unsupported K1 sign constraint in checkpoint: {k1_sign_constraint!r}"
                    )
                raw_k1_steps.append(raw_k1)
                applied_k1_steps.append(applied_k1)
                raw_k2_steps.append(raw_k2)
                td = env.step(action_td)["next"]
                trajectory.append(_positions(env))
                support, strength = _support_diagnostics(env, td)
                exported_support_index.append(support)
                exported_contact_strength.append(strength)
            ...
            metrics["seed"] = seed
            raw_k1_array = np.asarray(raw_k1_steps, dtype=np.float64)
            applied_k1_array = np.asarray(applied_k1_steps, dtype=np.float64)
            raw_k2_array = np.asarray(raw_k2_steps, dtype=np.float64)
            metrics["k1_sign_constraint"] = k1_sign_constraint
            metrics["canonical_raw_k1_sign_fraction"] = float(
                np.mean(raw_k1_array * canonical_k1_signs.reshape(1, 8) >= 0.0)
            )
            metrics["canonical_applied_k1_sign_fraction"] = float(
                np.mean(applied_k1_array * canonical_k1_signs.reshape(1, 8) >= 0.0)
            )
            metrics["mean_raw_k1_by_joint"] = np.mean(raw_k1_array, axis=0).tolist()
            metrics["mean_applied_k1_by_joint"] = np.mean(applied_k1_array, axis=0).tolist()
            metrics["mean_raw_k2_by_joint"] = np.mean(raw_k2_array, axis=0).tolist()
    ...
    loose_speeds = [
        episode["forward_speed_x100"]
        for episode in episodes
        if episode["loose_360_detected"]
    ]
    aggregate = {
        ...
        "k1_sign_constraint": k1_sign_constraint,
        "environment_k1_sign_constraint_validated": bool(
            k1_sign_environment_validated
        ),
        "mean_canonical_raw_k1_sign_fraction": float(
            np.mean([episode["canonical_raw_k1_sign_fraction"] for episode in episodes])
        ),
        "mean_canonical_applied_k1_sign_fraction": float(
            np.mean([episode["canonical_applied_k1_sign_fraction"] for episode in episodes])
        ),
        "mean_applied_k1_by_joint": np.mean(
            np.asarray([episode["mean_applied_k1_by_joint"] for episode in episodes]),
            axis=0,
        ).tolist(),
        "mean_raw_k2_by_joint": np.mean(
            np.asarray([episode["mean_raw_k2_by_joint"] for episode in episodes]),
            axis=0,
        ).tolist(),
        ...
        "mean_forward_speed_x100": float(
            np.mean([episode["forward_speed_x100"] for episode in episodes])
        ),
        "median_forward_speed_x100": float(
            np.median([episode["forward_speed_x100"] for episode in episodes])
        ),
        "loose_360_success_episodes": int(
            sum(bool(episode["loose_360_detected"]) for episode in episodes)
        ),
        "loose_360_success_rate": float(
            np.mean([bool(episode["loose_360_detected"]) for episode in episodes])
        ),
        "mean_best_fit_rotation_range_degrees": float(
            np.mean([episode["best_fit_rotation_range_degrees"] for episode in episodes])
        ),
        "median_best_fit_rotation_range_degrees": float(
            np.median([episode["best_fit_rotation_range_degrees"] for episode in episodes])
        ),
        "max_best_fit_rotation_range_degrees": float(
            np.max([episode["best_fit_rotation_range_degrees"] for episode in episodes])
        ),
        "mean_max_abs_rotation_from_episode_start_degrees": float(
            np.mean([
                episode["max_abs_rotation_from_episode_start_degrees"]
                for episode in episodes
            ])
        ),
        "max_abs_rotation_from_episode_start_degrees": float(
        ...
        "mean_loose_360_forward_speed_x100": (
            float(np.mean(loose_speeds)) if loose_speeds else None
        ),
...
def _self_test() -> None:
    positive = _rotation_excursion_metrics(np.full(37, math.radians(10.0)))
    assert positive["loose_360_detected"] and positive["first_360_direction"] == "ccw", positive
    negative = _rotation_excursion_metrics(np.full(37, math.radians(-10.0)))
    assert negative["loose_360_detected"] and negative["first_360_direction"] == "cw", negative
    return_to_start = _rotation_excursion_metrics(
        np.r_[np.full(37, math.radians(10.0)), np.full(37, math.radians(-10.0))]
    )
    assert return_to_start["loose_360_detected"], return_to_start
    rocking = _rotation_excursion_metrics(
        np.tile(np.r_[np.full(18, math.radians(10.0)), np.full(18, math.radians(-10.0))], 5)
    )
    assert not rocking["loose_360_detected"], rocking
    assert not _rotation_excursion_metrics(np.asarray([math.radians(359.9)]))["loose_360_detected"]
    assert _rotation_excursion_metrics(np.asarray([math.radians(360.0)]))["loose_360_detected"]
    ...
    print(
        "Self-test passed: bidirectional loose-360 excursion and true pulse detected; "
        "rocking, crawl, fixed-contact, and airborne cases rejected; environment "
        "support preferred with legacy fallback."
    )
