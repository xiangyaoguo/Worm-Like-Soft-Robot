...
OUTPUT_PATH = REPORT_ROOT / "Formal_HPR_Rolling_Mechanism_Validation_Bilingual_20260810.docx"

TABLE_HELPER_ROOT = Path(
    r"C:\Users\PUBLIC_USER\.codex\plugins\cache\openai-primary-runtime\documents"
    r"\26.805.11740\skills\documents\scripts"
)
if str(TABLE_HELPER_ROOT) not in sys.path:
    sys.path.insert(0, str(TABLE_HELPER_ROOT))
...
def configure_styles(doc: Document) -> None:
    ...
    chinese = doc.styles.add_style("Chinese Translation", 1)
    chinese.font.name = "Calibri"
    chinese._element.rPr.rFonts.set(qn("w:ascii"), "Calibri")
    chinese._element.rPr.rFonts.set(qn("w:hAnsi"), "Calibri")
    chinese._element.rPr.rFonts.set(qn("w:eastAsia"), "Microsoft YaHei")
    chinese.font.size = Pt(9.5)
    chinese.font.color.rgb = RGBColor.from_string(MID_GRAY)
    chinese.paragraph_format.space_before = Pt(0)
    chinese.paragraph_format.space_after = Pt(8)
    chinese.paragraph_format.line_spacing = 1.08
...
def add_bilingual_heading(doc: Document, english: str, chinese: str, level: int = 1) -> None:
    paragraph = doc.add_heading(level=level)
    paragraph.add_run(f"{english} / {chinese}")


def add_bilingual_paragraph(
    doc: Document,
    english: str,
    chinese: str,
    *,
    bold_lead_en: str | None = None,
    bold_lead_zh: str | None = None,
) -> None:
    p_en = doc.add_paragraph()
    if bold_lead_en and english.startswith(bold_lead_en):
        lead = p_en.add_run(bold_lead_en)
        set_run_font(lead, bold=True, colour=DARK_BLUE)
        p_en.add_run(english[len(bold_lead_en) :])
    else:
        p_en.add_run(english)
    p_zh = doc.add_paragraph(style="Chinese Translation")
    label = p_zh.add_run("English translation: ")
    set_run_font(label, size=9.5, bold=True, colour=BLUE)
    if bold_lead_zh and chinese.startswith(bold_lead_zh):
        lead = p_zh.add_run(bold_lead_zh)
        set_run_font(lead, size=9.5, bold=True, colour=MID_GRAY)
        p_zh.add_run(chinese[len(bold_lead_zh) :])
    else:
        p_zh.add_run(chinese)


def add_callout(doc: Document, english: str, chinese: str, colour: str = BLUE) -> None:
    ...
    run = paragraph.add_run("English translation: " + chinese)
...
def add_table(
    ...
    run = caption.add_run(caption_en)
    set_run_font(run, size=9, bold=True, colour=DARK_BLUE)
    run = caption.add_run(" / " + caption_zh)
    set_run_font(run, size=9, colour=MID_GRAY)
...
def add_figure(
    ...
    doc_pr.set("descr", caption_en)
    caption = doc.add_paragraph(style="Caption")
    caption.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = caption.add_run(caption_en)
    set_run_font(run, size=9, bold=True, colour=DARK_BLUE)
    run = caption.add_run("\n" + caption_zh)
    set_run_font(run, size=8.7, colour=MID_GRAY)
...
def configure_title_page(doc: Document) -> None:
    ...
    run = descriptor.add_run(
        ...
        "English translation: paired K₁/K₂ and joint-freezing interventions on two independent HPR training runs initialised with random seeds 9201 and 9205"
    )
...
def build_report() -> Path:
    ...
    add_bilingual_heading(doc, "Executive summary", "Executive summary", 1)
    ...
    doc.add_page_break()
    add_bilingual_heading(doc, "1. Scope and evidence boundary", "1. Scope and evidence boundary", 1)
    add_bilingual_paragraph(
        doc,
    ...
    add_bilingual_heading(doc, "2. Methods", "2. Methods", 1)
    add_bilingual_heading(doc, "2.1 Formal training checkpoints and controller", "2.1 Formal training checkpoints and controller", 2)
    add_bilingual_paragraph(
        doc,
    ...
    add_table(
        ...
        [
            ["run (random seed 9201)", "HPR", "training checkpoint 1500", "b697425f…183e7ea6"],
            ["run (random seed 9205)", "HPR", "training checkpoint 1500", "0edb440a…62eb9383"],
        ],
        [1.1, 0.8, 1.4, 3.2],
    ...
    add_bilingual_heading(doc, "2.2 Paired intervention design", "2.2 Paired intervention design", 2)
    add_bilingual_paragraph(
        doc,
    ...
    add_bilingual_heading(doc, "2.3 Rolling endpoint", "2.3 Rolling endpoint", 2)
    add_bilingual_paragraph(
        doc,
    ...
    add_bilingual_heading(doc, "3. Integrity and baseline verification", "3. Integrity and baseline verification", 1)
    baseline_rows = []
    for seed in (9201, 9205):
    ...
    add_bilingual_heading(doc, "4. Results", "4. Results", 1)
    add_bilingual_heading(doc, "4.1 Global feedback-channel interventions", "4.1 Global feedback-channel interventions", 2)
    global_table_rows = []
    for seed in (9201, 9205):
        for condition in ("BASELINE", "GLOBAL_K1_OFF", "GLOBAL_K2_OFF", "GLOBAL_BOTH_OFF"):
    ...
    add_bilingual_heading(doc, "4.2 Whole-joint ablation and single-joint retention", "4.2 Whole-joint ablation and single-joint retention", 2)
    ...
    add_bilingual_heading(doc, "4.3 Joint-specific K₁ and K₂ contributions", "4.3 Joint-specific K₁ and K₂ contributions", 2)
    ...
    add_bilingual_heading(doc, "4.4 Cross-policy agreement", "4.4 Cross-policy agreement", 2)
    ...
    add_bilingual_heading(doc, "5. Representative morphology and temporal evidence", "5. Representative morphology and temporal evidence", 1)
    add_bilingual_paragraph(
        doc,
    ...
    add_bilingual_heading(doc, "6. Interpretation and thesis claim", "6. Interpretation and thesis claim", 1)
    add_bilingual_paragraph(
        doc,
    ...
    doc.add_page_break()
    add_bilingual_heading(doc, "7. Conclusion", "7. Conclusion", 1)
    add_bilingual_paragraph(
        doc,
    ...
    add_bilingual_heading(doc, "8. Copy-ready thesis paragraph", "8. Copy-ready thesis paragraph", 1)
    thesis_en = (
    ...
    add_bilingual_paragraph(doc, thesis_en, thesis_en)

    doc.add_page_break()
    add_bilingual_heading(doc, "9. Limitations and remaining experiments", "9. Limitations and remaining experiments", 1)
    limitations = [
        (
            "Training-run scope",
    ...
    for en_label, en_text, zh_label, zh_text in limitations:
        add_bilingual_paragraph(
            doc,
            f"{en_label}: {en_text}",
            f"{en_label}: {en_text}",
            bold_lead_en=f"{en_label}: ",
            bold_lead_zh=f"{zh_label}: ",
        )

    add_bilingual_heading(doc, "10. Reproducibility index", "10. Reproducibility index", 1)
    add_bilingual_paragraph(
        doc,
