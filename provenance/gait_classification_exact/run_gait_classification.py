...
def validate_inputs(input_root: Path) -> tuple[list[dict[str, str]], list[Path], list[dict[str, Any]]]:
    ...
    if strict_count != 146 or lenient_count != 147:
...
def validate_locked_classification(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
"""
    ...
    english_report = f"""# Supplemental Crawling-Experiment Results Summary

## Experiment Status

Automated candidate classification has completed for six configurations, five independent training runs per configuration, and 20 paired resets per frozen policy: 600 trajectories of 1,000 steps each. Original checkpoint-1500 artifacts were read-only, and input hashes matched before and after analysis. Two-person anonymous blinded review is still required before a `crawling candidate` can be promoted to `human-confirmed crawling`.

## Automated-Stage Aggregate Results
...
{markdown_table(['Configuration', 'Crawl candidates/100', 'Formal rolling/100', 'Partial rolling/100', 'Failure/other/100', 'Stable crawl runs'], chinese_table)}

## Conclusions Safe to Report in the Thesis

Under the locked automated criteria, all five independent HPR-O2-PS training runs produced crawling candidates on all 20 resets (100/100), with no formal rolling. SGRR-O2-JS instead primarily produced formal rolling ({next(row['formal_rolling_count_total'] for row in config_rows if row['configuration_id'] == 'SGRR_O2_JS')}/100), with no crawling candidates. HPR-O2-JS produced a mixture of crawling candidates, formal rolling, and partial rolling, showing that the learned gait composition diverged after changing joint-sharing architecture. These are descriptive results for frozen policies under a fixed evaluation protocol and must not be presented as causal conclusions.
...
The 600 rollouts are not 600 independent replicates. The independent units for configuration comparisons are the five training runs per configuration; the 20 resets are nested environment states paired across policies. Formal crawling success rates must await anonymous labels from two independent reviewers and adjudication of disagreements. Until then, all prose must use `crawling candidate` or `automated crawling candidate`.

In addition, all 30 checkpoint SHA-256 values differ, but there are only 23 distinct full run-level position matrices. Two groups of respectively five and four independently trained checkpoints converge to exactly the same effective deterministic behavior. All runs are retained and disclosed separately in the QA table; they must not be removed as duplicate files.
"""
    (paths["report"] / "experiment_results_summary_en.md").write_text(english_report, encoding="utf-8")
...
def main() -> None:
    ...
    episode_rows, npz_files, hash_rows = validate_inputs(args.input_root)
    ...
    classification_qa = validate_locked_classification(rows)
