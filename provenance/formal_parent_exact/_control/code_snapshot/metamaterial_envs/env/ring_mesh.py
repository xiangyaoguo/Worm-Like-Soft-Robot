...
class env(EnvBase):
    ...
    def render(self):
        ...
        # ground
        for line in LINES:
            points = line - self.camera_pos
            points = np.conjugate(points * scale) + offset
            p0 = np.array([np.real(points[0]), np.imag(points[0])])
            p1 = np.array([np.real(points[1]), np.imag(points[1])])
            pygame.draw.line(canvas, 0, p0, p1, width=3)

            for node in self.pos[0]:
                v = node.item() - line[0]
                u = line[1] - line[0]
                v_onto_u = u * np.real(v / u)
                p_onto_u = line[0] + v_onto_u
                p = p_onto_u - self.camera_pos
                p = np.conjugate(p * scale) + offset
                pygame.draw.circle(canvas, (200, 200, 200), np.array([np.real(p), np.imag(p)]), 5)
