...
LEGACY_ROOT = ROOT.parent / "mechanism_runtime_exact"
...
def classification_markdown(
    ...
    lines = [
        "# K1/K2 Causal Completion: Contract-Adjudicated Results",
        "",
        "> This file is generated automatically by `analyze_causal_completion_results.py` under the frozen contract. The independent inference units are five training seeds; the 20 initial states per seed are paired repeats, not 100 independent policy samples.",
        "",
        "## Evidence Integrity",
        "",
        ...
        f"- Strictly paired C00: {validation['matched_c00']['training_seed_count']} training seeds x {validation['matched_c00']['episodes_per_training_seed']} episodes, {validation['matched_c00']['episode_count']} episodes total; shares evaluation seeds 20264401-20264420 with the new matrix.",
        f"- Archived legacy evidence: {validation['legacy']['condition_count']} conditions and {validation['legacy']['episode_count']} episodes.",
        "- Every success label is recomputed and checked episode by episode against the joint pulse, rotation, direction, forward-displacement, and pulse-interval thresholds.",
        "",
        "## Frozen Adjudication Rules",
        "",
        f"- Strong necessity: zeroing in C11 lowers success by at least {100*rule['strong_drop']:.0f} percentage points and lowers it in at least {rule['strong_seeds']}/5 training seeds.",
        f"- Necessary contribution: a drop of at least {100*rule['necessary_drop']:.0f} percentage points in at least {rule['necessary_seeds']}/5 training seeds.",
        f"- Timing-critical: static mean or time-permuted intervention lowers success by at least {100*rule['timing_drop']:.0f} percentage points in at least {rule['timing_seeds']}/5 seeds.",
        f"- Sufficient: gain over C00 of at least {100*rule['suff_gain']:.0f} percentage points, with at least {rule['suff_seeds_reach']}/5 seeds reaching 10/20.",
        f"- Equivalent/redundant: absolute success-rate difference no greater than {100*rule['equivalent_rate']:.0f} percentage points, every primary continuous-metric difference within {rule['equivalent_sd']:.1f} reference SD, and no consistent degradation in at least 4/5 seeds.",
        "",
        "## Per-Joint, Per-K Adjudication",
        "",
        "|Channel|Zeroing difference (pp)|Strongly necessary|Necessary contribution|Static difference (pp)|Permuted difference (pp)|Timing-critical|Legacy C00 single-channel sufficient|Zeroing equivalent/redundant|",
        "|---|---:|:---:|:---:|---:|---:|:---:|:---:|:---:|",
    ]
    for _, row in cards.iterrows():
        lines.append(
            "|{channel}|{zero:.1f}|{strong}|{necessary}|{static:.1f}|{permuted:.1f}|{timing}|{suff}|{equiv}|".format(
                ...
                strong=chinese_bool(row["strong_necessity"]),
                necessary=chinese_bool(row["necessary_contribution"]),
                ...
                timing=chinese_bool(row["timing_critical"]),
                suff=chinese_bool(row["legacy_single_channel_sufficient"]),
                equiv=chinese_bool(row["zero_equivalent_or_redundant"]),
            )
        )
    lines.extend(
        [
            ...
            "",
            "|Joint|Whole-pair zeroing difference (pp)|Strongly necessary|Necessary contribution|Whole-pair transplant gain (pp)|Seeds reaching 10/20|Sufficient|",
            "|---|---:|:---:|:---:|---:|---:|:---:|",
        ]
    )
    for _, row in pairs.iterrows():
        lines.append(
            "|{joint}|{nec:.1f}|{strong}|{necessary}|{suff:.1f}|{seeds}|{sufficient}|".format(
                ...
                strong=chinese_bool(row["strong_joint_pair_necessity"]),
                necessary=chinese_bool(row["joint_pair_necessary_contribution"]),
                suff=row["sufficiency_success_rate_gain_percentage_points"],
                seeds=int(row["sufficiency_training_seeds_reaching_10_of_20"]),
                sufficient=chinese_bool(row["joint_pair_sufficient"]),
            )
        )
    lines.extend(
        [
            ...
            "",
            "- Single-channel sufficiency comes from the archived legacy 59-condition matrix; candidates and C00 use the same legacy evaluation initial states.",
            "- New whole-pair sufficiency conditions and matched C00 use the same training seeds and the same ordered initial states 20264401-20264420, so joint sufficiency is a strictly paired comparison. Legacy C00 is only a historical reproduction control and does not enter the primary new joint-sufficiency adjudication.",
            "- Primary continuous metrics for equivalence were fixed in advance as forward body lengths, target net rotation, direction ratio, rolling-pulse count, and mean pulse interval; each is standardized by the episode-level SD of the reference condition.",
            "- `success rate` is the equally weighted mean of five seed-level success rates. Episode-level failure rates in the CSV are only for mechanism description; the 100 episodes cannot be treated as independent n=100 policy-level observations.",
            "",
            "## Figures",
            "",
        ]
