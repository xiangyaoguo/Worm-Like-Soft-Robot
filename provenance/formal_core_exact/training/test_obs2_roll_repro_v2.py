...
def assert_reward_only_contract_and_batch_independence() -> None:
    rewards = ("horizontal_speed", "obs2_roll_repro_v1", "obs2_roll_repro_v2")
    envs = [make_env(name, randomness=0.01) for name in rewards]
    batch_zero = make_env("obs2_roll_repro_v2", randomness=0.01)
    batch_late = make_env("obs2_roll_repro_v2", randomness=0.01)
    try:
        reset_tds = []
...
def assert_parser_contract() -> None:
    ...
    try:
        sys.argv = [
            ...
            "--reward-func", "obs2_roll_repro_v2",
            "--per-joint-k1-k2",
        ...
        sys.argv = original
    assert args.reward_func == "obs2_roll_repro_v2"
...
def assert_tail_ability_and_launch_gate() -> None:
    test_env = make_env("obs2_roll_repro_v2")
    try:
        test_env.reset()
    ...
    airborne = make_env("obs2_roll_repro_v2")
    try:
        airborne.reset()
    ...
    takeoff = make_env("obs2_roll_repro_v2")
    try:
        takeoff.reset()
...
def assert_adversaries_valid_pulse_cache_and_reset() -> None:
    ...
    rolling = make_env("obs2_roll_repro_v2")
    try:
        rolling.reset()
        ready = tail_first_shape(curvature_degrees=10.0)
        for _ in range(8):
...
def assert_multi_env_state_isolation() -> None:
    test_env = make_env("obs2_roll_repro_v2", num_envs=2)
    try:
        test_env.reset()
...
def main() -> None:
    ...
    print("PASS: obs2_roll_repro_v2 reward-only contract and adversarial tests")
