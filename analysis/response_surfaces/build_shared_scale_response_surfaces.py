...
ROOT = Path(os.environ.get("THESIS_LEGACY_DATA_ROOT", str(SOURCE_DIR)))
ATLAS = (
    ROOT
    / "05_ExperimentalDataAndCode"
...
OUT_DIR = Path(
    os.environ.get("THESIS_FIGURE_OUTPUT", str(SOURCE_DIR / "generated"))
).resolve()
...
SOURCE_SELECTED_DATA = SOURCE_DIR / "Figure_HPR_SGRR_run0_K1_K2_selected_data.npz"
...
def load_inputs() -> tuple[np.ndarray, np.ndarray, str]:
    if SOURCE_SELECTED_DATA.is_file():
        with np.load(SOURCE_SELECTED_DATA) as payload:
            surfaces = np.stack(
                (
                    np.asarray(payload["hpr_surfaces"], dtype=np.float32),
                    np.asarray(payload["sgrr_surfaces"], dtype=np.float32),
                ),
                axis=0,
            )
            observations = np.stack(
                (
                    np.asarray(payload["hpr_observation"], dtype=np.float32),
                    np.asarray(payload["sgrr_observation"], dtype=np.float32),
                ),
                axis=0,
            )
        ...
        if observations.shape != (2, 1000, 8, 2):
            raise RuntimeError(f"Unexpected selected observation shape: {observations.shape}")
        return surfaces, observations, "bundled_selected_data"
    ...
    return surfaces, observations, "legacy_atlas_and_traces"
...
def main() -> None:
    ...
    surfaces, observations, input_mode = load_inputs()
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "selected_data_source": {
                "path": str(SOURCE_SELECTED_DATA),
                "sha256": sha256_file(SOURCE_SELECTED_DATA),
            },
