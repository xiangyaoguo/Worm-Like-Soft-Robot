...
> **Portable-release note:** the absolute paths below document the archived operator procedure. On a new machine use `python run.py train --mode ... --arm HPR-O2-AVM-JS` for a portable scientific rerun and `python run.py evaluate-avm` for endpoint evaluation. The portable trainer does not claim the archived bitwise initialization/approval gates; see the root `README.md` for the archive-exact phases.
