...
def first_not_none(*values):
    """Return the first non-None metadata value, preserving falsey values."""
    for value in values:
...
def install_actor_observation_intervention(policy, metadata: dict) -> None:
    """Restore the thesis AVM actor-only observation mask when required.

    AVM checkpoints keep the physical two-channel environment and K2 term, but
    replace the actor's theta-dot input with exact zeros.  The historical demo
    predated that extension, so the portable release reinstalls the same
    parameter-free pre-hook from checkpoint metadata before replay.
    """

    mode = str(metadata.get("actor_observation_mode", "full_o2"))
    if mode == "full_o2":
        return
    if mode != "spatial_only_sham":
        raise RuntimeError(f"Unsupported actor_observation_mode: {mode!r}")
    candidates = [module for module in policy.modules() if isinstance(module, MultiAgentMLP)]
    if len(candidates) != 1:
        raise RuntimeError(
            "AVM replay expected exactly one MultiAgentMLP actor backbone; "
            f"found {len(candidates)}"
        )

    def spatial_only_pre_hook(module, inputs):
        if len(inputs) != 1 or not torch.is_tensor(inputs[0]):
            raise RuntimeError("AVM actor backbone expected one tensor input")
        observation = inputs[0]
        if observation.shape[-1] != 2:
            raise RuntimeError(
                "AVM requires the locked two-channel O2 actor input; "
    ...
    candidates[0]._thesis_avm_hook = candidates[0].register_forward_pre_hook(
        spatial_only_pre_hook
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preflight",
        action="store_true",
        help="Load the checkpoint, rebuild the policy, run one headless step, and exit.",
    )
        ...
        if metadata.get("control_mode") == "tail_wave_residual":
            policy_net = torch.nn.Sequential(
                ...
                ScratchWRDeterministicParamExtractor(
                    ...
                    alpha=float(
                        first_not_none(
                            metadata.get("scratch_wr_current_alpha"),
                            metadata.get("scratch_wr_initial_alpha"),
                            0.0,
                        )
                    ),
                ),
...
def main() -> None:
    ...
    meta_env = metamaterial.env(
        ...
        render_mode=None if args.preflight else "human",
        observation_func=observation_func,
    ...
    if hasattr(meta_env, "set_curriculum_episode"):
        meta_env.set_curriculum_episode(
            int(first_not_none(metadata.get("scratch_wr_current_batch"), 0))
        )
    ...
    install_actor_observation_intervention(policy, metadata)
    try:
    ...
    if args.preflight:
        ...
        print(f"Preflight passed: one headless step completed (speed={speed:.6f}).")
